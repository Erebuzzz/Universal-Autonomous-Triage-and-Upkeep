...
export function FlipCard({
  ...
  const sumX = useTransform([turn, tiltX], ([t, x]) => Number(t) + Number(x));
  const sumY = useTransform([turn, tiltY], ([t, y]) => Number(t) + Number(y));
  ...
  const facing = useTransform(turn, (t) => Math.abs(Math.cos((Number(t) * Math.PI) / 180)));
  const spread = useTransform(facing, (f) => 0.08 + 0.92 * Number(f));
  const shade = useTransform(facing, (f) => 0.1 + 0.9 * Number(f) * Number(f));
  ...
  const rotorStyle = {
    ...
    ["--fc-gx" as string]: gxPct,
    ["--fc-gy" as string]: gyPct,
    ["--fc-sheen" as string]: sheen,
  };
  ...
  return (
    <div
      ...
      style={{
        ["--fc-w" as string]: `${width}px`,
        ["--fc-h" as string]: `${height}px`,
        ["--fc-radius" as string]: `${radius}px`,
    ...
    >
      ...
      <motion.div className="flip-card__rotor" style={reduce ? undefined : rotorStyle}>
        <div className="flip-card__face flip-card__face--front" aria-hidden={shown}>
          {front}
          {glare ? <span className="flip-card__glare" aria-hidden="true" /> : null}
        </div>
        <div className="flip-card__face flip-card__face--back" aria-hidden={!shown}>
          {back}
          {glare ? <span className="flip-card__glare" aria-hidden="true" /> : null}
        </div>
      </motion.div>
