...
const PATTERNS: Record<string, Record<number, PatternSpec>> = {
...
const resolvePattern = (pattern: string | PatternSpec, grid: GridSize): PatternSpec => {
...
export function LatticeLoader({
  label = "Triaging",
  doneLabel = "Remediated in",
  errorLabel = "Defect at",
  status = "working",
  pattern = "orbit",
  grid = 3,
  shape = "round",
  ...
  if (status !== "working") markRef.current = status;
  ...
  useEffect(() => {
    ...
    else setAnnounce(`${status === "done" ? doneLabel : errorLabel}${showTimer ? ` ${fmt(dsRef.current)}` : ""}`);
  }, [status, doneLabel, errorLabel, label, showTimer]);
  ...
  return (
    <span
      ...
      style={{
        ["--ll-n" as string]: n,
        ["--ll-cell" as string]: `${cellSize}px`,
        ["--ll-gap" as string]: `${gap}px`,
        ["--ll-font" as string]: `${fontSize}px`,
        ["--ll-color" as string]: color,
        ["--ll-mark" as string]: status === "error" ? errorColor : doneColor,
        ["--ll-idle" as string]: idleOpacity,
        ["--ll-glow" as string]: glowColor || color,
        ["--ll-mark-glow" as string]: glowColor || (status === "error" ? errorColor : doneColor),
        ["--ll-cycle" as string]: `${cycle}ms`,
        ...style,
      }}
    >
